...
def manage_apis_view(id):
    return _manage_associated_packages_view(
        ...
        add_success_singular="Το API προστέθηκε στην επανάχρηση.",
        add_success_plural="Τα APIs προστέθηκαν στην επανάχρηση.",
        remove_success_singular="Το API αφαιρέθηκε από την επανάχρηση.",
        remove_success_plural="Τα APIs αφαιρέθηκαν από την επανάχρηση.",
    ...
    if tk.request.method == 'POST':
        ...
        if showcase_id:
            ...
            if not showcase['approval_status'] == 'approved':
                return tk.abort(
                    403,
                    _('Δεν μπορεί να γίνει αφαίρεση διασύνδεσης μη εγκεκριμένης επανάχρησης με σύνολο δεδομένων.'),
                )
            data_dict = {
                "showcase_id": showcase_id,
